...
# Right now uses DUMMY data hardcoded below.
# ─────────────────────────────────────────────────────────────
# WHEN REAL API IS READY:
#   Replace the three DATA sections (PROGRAMMES, PAPERS, PDFS)
#   with real API calls. Everything else stays the same.
# ─────────────────────────────────────────────────────────────
# Run: python seed.py
...
# ═══════════════════════════════════════════════════════════════
# DATA  ← swap these three dicts with real API calls later
# ═══════════════════════════════════════════════════════════════
...
PDFS = [
    # Paper 1 — History of Assamese Lit (3 blocks)
    ...
    # Paper 3 — Assamese Language (2 blocks)
    ...
    # Paper 9 — Folk Literature (2 blocks)
    ...
    # Paper 12 — Computer Fundamentals (2 blocks)
    ...
    # Paper 18 — Web Technology (2 blocks)
    ...
    # Paper 21 — Business Organisation (2 blocks)
    ...
    # Paper 25 — European Classical Lit (2 blocks)
...
def seed():
    ...
    for p in PROGRAMMES:
        conn.execute(
            "INSERT OR IGNORE INTO programmes (id, code, name) VALUES (?, ?, ?)",
            (p["id"], p["code"], p["name"])
        )
    ...
    # ── PDFs ──
    # Build organised file paths for when real PDFs arrive
    for pdf in PDFS:
        # Find the paper to build a clean folder path
        ...
        conn.execute(
            """INSERT OR IGNORE INTO pdfs (paper_id, title, pdf_name, file_path, status)
               VALUES (?, ?, ?, ?, 'pending')""",
            (pdf["paper_id"], pdf["title"], pdf["pdf_name"], file_path)
        )

    print(f"  ✓ {len(PDFS)} PDFs seeded (status: pending — upload real files to activate)")
